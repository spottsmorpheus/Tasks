...
Function Out-UiLog {
    ...
    Begin {
        ...
        $LogOutput = [System.Collections.Generic.List[Object]]::new()
    }
    Process {
        # Process Pipe
        ...
        $LogData | 
            Where-Object $Filter |
            Select-Object -Property lineNumber, timestamp, thread, level, class, @{n="message";e={[String]::Join("`n",$_.message)}} |
            Foreach-Object {$LogOutput.Add($_)}
    }
    End {
        #Formatted Log data in $LogOutput
        $LogOutput | Out-HtmlPage -Title "Parse-MorpheusUILog Output" -Path $Path
    }

}
# Use Powershell to drive remote ssh session to grap live UI logs off Appliance
Function Get-RemoteUiLog {
    [CmdletBinding()]
    Param (
        [String]$sshHost,
        [String]$Start="",
    ...
    # $StartTime and $EndTime [DateTime] variables are availale for filtering

    Write-Warning "Using ssh to execute remote command on $sshHost. It may propmt for Password"
    $cmd = "ssh -t {0} ""sudo sed -n '/^{1}/,/^{2}/p' /var/log/morpheus/morpheus-ui/current"" " -f $sshHost, $StartTime.ToString("yyyy-MM-dd_HH:mm"),$EndTime.ToString("yyyy-MM-dd_HH:mm")
